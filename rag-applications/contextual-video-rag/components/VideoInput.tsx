...
export const VideoInput: React.FC<VideoInputProps> = ({
  ...
  const extractFrames = useCallback(
    async (videoFile: File): Promise<FrameData[]> => {
      ...
      return new Promise((resolve, reject) => {
        ...
        video.onloadedmetadata = async () => {
          ...
          if (duration === 0 || !isFinite(duration) || video.videoWidth === 0 || video.videoHeight === 0) {
            ...
            reject(
              new Error(
                'Could not determine video duration or dimensions. The video file might be corrupt or unsupported.'
              )
            );
            return;
          }

          // create evenly-spaced timestamps between start and end
          const timePoints: number[] = [];
          if (maxFrames <= 1) {
            timePoints.push(Math.min(0.1, duration / 2));
          } else {
            for (let i = 0; i < maxFrames; i++) {
              timePoints.push(Math.max(0, (i / (maxFrames - 1)) * (duration - 0.1)));
            }
          }
              ...
              const onSeeked = () => {
                ...
                if (ctx) {
                  try {
                    ...
                    const base64Data = canvas.toDataURL(mimeType, 0.85).split(',')[1];
                    frames.push({ id: `frame-${framesExtracted}`, base64Data, mimeType, timestamp: time });
                    framesExtracted++;
                    resolveFrame();
                  } catch (e) {
            ...
            try {
              for (const time of timePoints) {
                if (framesExtracted < maxFrames) {
                  await captureFrameAtTime(Math.max(0, Math.min(time, duration - 0.01)));
                }
              }
              URL.revokeObjectURL(video.src);
              resolve(frames);
      ...
      if (file) {
        ...
        try {
          ...
          if (extractedFrames.length === 0) {
            throw new Error(
              'No frames could be extracted. The video might be too short or in an unsupported format.'
            );
          }
